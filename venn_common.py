...
# Normalize publication titles: remove punctuation, extra spaces, and lowercase
def clean_title(title):
    if not title or not isinstance(title, str):
        return ""
...
# Remove the .pdf extension from file names
def clean_filename(file_name):
...
##########################################################################
# Updated extraction function using pdfplumber and optional citation field
##########################################################################
def extract_titles_citations_years(file_object):
    """
    Extracts (Title, Year, Citations) from a PDF.
    Uses pdfplumber to extract the full text (saved to 'extracted_text.txt' for debugging)
    and applies a regex that allows an optional citation field.
    If citation is missing, it is set to 0.
    Expected formats:
       Some Publication Title ... 611 2011
       Some Publication Title Without Citation 2015
    """
    if file_object is None or file_object.getbuffer().nbytes == 0:
        return []
...
##########################################################################
# Fuzzy string matching to compare two sets of titles
##########################################################################
def find_similar_titles(set1, set2, threshold=THRESHOLD):
    matches = set()
    for title1 in set1:
...
##########################################################################
st.title("Google Scholar PubMatch")
st.markdown('"This common publications matching tool is implemented based on the work done by Rohit Ramachandran, Rutgers University, NJ, USA"')
st.markdown("""
### Instructions:
1. Upload at least 2 Google Scholar PDFs (printed via "Print to PDF").
2. The algorithm extracts publication titles, citation counts, and publication years.
3. It then computes common publications among researchers using an order-independent approach.
4. Results include a comparison table, summary statistics, an UpSet plot, and (for 2–4 researchers) a Venn diagram.
""")
...
if "All Years" in selected_years:
    ...
    selected_years = set(str(year) for year in selected_years if year != "All Years")

uploaded_files = st.file_uploader("Upload Google Scholar PDFs of researchers", type=["pdf"], accept_multiple_files=True)
run_simulation_button = st.button("Run Simulation")
    ...
    if uploaded_files:
        ...
        for file in uploaded_files:
            if is_google_scholar(file):
                ...
                filtered_titles = {(title, year, int(citations))
                                   for (title, year, citations) in extracted_data
                                   if year in selected_years}
                researcher_data[researcher_name] = filtered_titles
            else:
                st.warning(f"Skipping {file.name}: Not recognized as a Google Scholar PDF.")
        
        if len(researcher_data) > 1:
            st.subheader("Common Publications Between Researchers")
            
            # Build a dictionary of sets for each researcher (titles only)
            all_sets = {name: set(t[0] for t in titles if t[0] != "N/A")
                        for name, titles in researcher_data.items()}
            # Preserve full data for citation lookups
            all_data_with_details = {name: titles for name, titles in researcher_data.items()}
            comparisons = []

            # ----- Order-independent common publication calculation -----
            for r in range(2, len(all_sets) + 1):
                for comb in combinations(all_sets.keys(), r):
                    # 1) Gather union of all candidate titles in this combination
                    union_titles = set()
                    for name in comb:
                        union_titles.update(all_sets[name])
                    
                    # Debug: show union for this combination
                    # st.write(f"Debug: Union of titles for combination {comb}: {sorted(union_titles)}")
                    
                    # 2) For each candidate in union_titles, check if it fuzzy-matches at least one title in EVERY researcher's set
                    common_titles = set()
                    for candidate in union_titles:
                        found_in_all = True
                        debug_ratios = {}
                        for name in comb:
                            ratios = [fuzz.ratio(candidate, t) for t in all_sets[name]]
                            max_ratio = max(ratios) if ratios else 0
                            debug_ratios[name] = max_ratio
                            if max_ratio < THRESHOLD:
                                found_in_all = False
                                break
                        # Optional: print debug info for candidate
                        #st.write(f"Candidate '{candidate}' ratios: {debug_ratios}")
                        if found_in_all:
                            common_titles.add(candidate)
                    
                    # 3) Sum citations for common titles
                    total_citations = 0
                    for title in common_titles:
                        citation_values = []
                        for researcher in comb:
                    ...
                    # st.write(f"Debug - Common Publications for {comb}: {sorted(common_titles)}")
            # ----- End of common publication calculation -----

            df_comparisons = pd.DataFrame(comparisons)
            df_comparisons.index = range(1, len(df_comparisons) + 1)
            st.dataframe(df_comparisons.drop(columns=["Common Publications"]).style.set_properties(
                **{"text-align": "left", "white-space": "pre-wrap"}
            ), height=(50 + len(df_comparisons)*35), width=800)

            @st.cache_data
            def convert_df(df):
                return df.to_csv(index=False).encode("utf-8")

            # Calculate total common publications and citations per researcher
            researcher_totals = {name: {"common_titles_with_citations": set()}
                                  for name in researcher_data.keys()}
            for _, row in df_comparisons.iterrows():
                researchers_in_comb = row["Combinations"].split(" ↔ ")
                common_titles_list = row["Common Publications"].split("|")
                for researcher in researchers_in_comb:
                    for candidate_title in common_titles_list:
                        ...
                        for r in researchers_in_comb:
                            matched_value = 0
                            for (t, y, c) in all_data_with_details[r]:
                                if fuzz.ratio(t, candidate_title) >= THRESHOLD:
                                    matched_value = c
                                    break
                            citation_values.append(matched_value)
                        chosen_citation = min(citation_values) if citation_values else 0
                        researcher_totals[researcher]["common_titles_with_citations"].add((candidate_title, chosen_citation))
            
            data_rows = []
            for researcher, data_dict in researcher_totals.items():
                unique_titles = set(t for t, c in data_dict["common_titles_with_citations"])
                total_researcher_citations = sum(c for t, c in data_dict["common_titles_with_citations"])
                data_rows.append((researcher, len(unique_titles), total_researcher_citations))
            df_totals = pd.DataFrame(data_rows, columns=["Researcher", "Total Common Publications", "Total Common Citations"])
            df_totals.index = range(1, len(df_totals) + 1)
            st.subheader("Total Common Publications and Citations per Researcher")
            st.dataframe(df_totals.style.set_properties(**{"text-align": "left", "white-space": "pre-wrap"}),
                         height=(50 + len(df_totals)*35), width=800)

            csv_data = convert_df(df_comparisons)
            st.download_button(label="Download Comparison as CSV",
                               data=csv_data,
                               file_name="publication_comparisons.csv",
                               mime="text/csv")
            ...
            for _, row in df_comparisons.iterrows():
                combination = row["Combinations"]
                num_common = row["No. of common publications"]
                combo_researchers = combination.split(" ↔ ")
                membership = tuple(r in combo_researchers for r in researcher_names)
                upset_data[membership] = upset_data.get(membership, 0) + num_common
            index = pd.MultiIndex.from_tuples(upset_data.keys(), names=researcher_names)
            data_counts = pd.Series(list(upset_data.values()), index=index)
            upset = UpSet(data_counts, subset_size="auto", show_counts="%d", sort_by="cardinality", facecolor="blue")
            fig = plt.figure(figsize=(12,8))
            ...
            # Generate Venn Diagram for 2-4 researchers
            if 2 <= len(researcher_data) <= 4:
                ...
                for r in range(1, len(researcher_names)+1):
                    for comb in combinations(researcher_names, r):
                        intersection_counts[comb] = 0
                for _, row in df_comparisons.iterrows():
                    combo_researchers = tuple(row["Combinations"].split(" ↔ "))
                    intersection_counts[combo_researchers] = row["No. of common publications"]
                singleton_counts = {}
                for name in researcher_names:
                    union_common = set()
                    for comb, count in intersection_counts.items():
                        if name in comb:
                            union_common.update(set())
                    # Here, instead of subtracting counts (which can lead to negatives),
                    # we compute singletons as those in all_sets[name] that are not in the union of all common publications for that researcher.
                    # Build union of common publications for this researcher from df_comparisons:
                    union_for_researcher = set()
                    for _, row in df_comparisons.iterrows():
                        combo = row["Combinations"].split(" ↔ ")
                        if name in combo:
                            union_for_researcher.update(row["Common Publications"].split("|"))
                    singleton_counts[name] = len(all_sets[name] - union_for_researcher)
                ...
                if len(researcher_names) == 2:
                    ...
                    venn_data = [
                        singleton_counts[name1],
                        singleton_counts[name2],
                        intersection_counts.get((name1, name2), 0)
                    ]
                    fig, ax = plt.subplots(figsize=(6,6))
                    v = venn.venn2(subsets=venn_data, set_labels=researcher_names)
                    st.pyplot(fig)
                elif len(researcher_names) == 3:
                    name1, name2, name3 = researcher_names
                    venn_data = [
                        ...
                        intersection_counts.get((name1, name2), 0),
                        singleton_counts[name3],
                        intersection_counts.get((name1, name3), 0),
                        intersection_counts.get((name2, name3), 0),
                        intersection_counts.get((name1, name2, name3), 0)
                    ]
                    fig, ax = plt.subplots(figsize=(6,6))
                    v = venn.venn3(subsets=venn_data, set_labels=researcher_names)
                    st.pyplot(fig)
                elif len(researcher_names) == 4:
                    name1, name2, name3, name4 = researcher_names
                    venn_sets = {name: set() for name in researcher_names}
                    for name in researcher_names:
                        ...
                        venn_sets[name].update([f"{name}_unique_{i}" for i in range(count)])
                    for (n1, n2) in combinations(researcher_names, 2):
                        count = intersection_counts.get((n1, n2), 0)
                        titles = [f"{n1}_{n2}_{i}" for i in range(count)]
                        venn_sets[n1].update(titles)
                        venn_sets[n2].update(titles)
                    for (n1, n2, n3) in combinations(researcher_names, 3):
